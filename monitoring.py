...
parser.add_argument('--hosts',  type=list, default=["127.0.0.1"], help='list of machine you want to monitor, eg: ["146.148.118.87", "127.0.0.1"]')
parser.add_argument('--gc_log_file',  type=str, default="/home/quentin/apache-cassandra/apache-cassandra-3.8/logs/gc.log.*.current", help='gc log file path, eg: "/opt/cassandra/logs/gc.log.*.current".  Change to None or "" to disable')
parser.add_argument('--log_file',  type=str, default="/home/quentin/apache-cassandra/apache-cassandra-3.8/logs/system.log", help='cassandra log file path, eg: "/var/log/system.log". Count errors and warns. Change to None or "" to disable')
...
class Host:
    def __init__(self, name):
        ...
        self.jvm_stop = -1
        self.error_count = -1
        self.previous_error_count = -1
        self.warn_count = -1
        self.previous_warn_count = -1
...
def execute_remote_command(host):
    try:
        ...
        print "connection error with host "+host+"... retry connection in 1 sec..."
    finally:
...
while threading.active_count() > 0:
    ...
    meta_header_1 += "|".ljust(9, " ")+"NETWORK (ALL INTERFACES EXPECT LO)".ljust(41, " ")+"|    JVM    |  LOGS (last "+str(args.log_grep_freq)+"sec)"
    ...
    try:
        ...
        observer.mutex.release()

    if args.small_screen:
        print args.small_screen
        print "SMALL"
